...
flags = tf.app.flags
flags.DEFINE_string('input_video', 'object_detection/video/170491918.avi', 'Input video to test.')
flags.DEFINE_string('export_model', 'inference_graph_vfss2', 'Add your trained model.')
FLAGS = flags.FLAGS

# Model preparation
export_model=FLAGS.export_model
MODEL_NAME = 'object_detection/{}'.format(export_model)
...
with detection_graph.as_default():
  with tf.Session(graph=detection_graph) as sess:
    ...
    recording_video = "rec_{}.avi".format(input_video[:-4]) # input_video: video name(sys.args[1])
    out = cv2.VideoWriter(recording_video, fcc, fps, (width, height))
    ...
    while True:
      ...
      # Display
      display_model_name = MODEL_NAME.split('/')[0]
